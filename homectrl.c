...
#define ON			1
#define OFF			0
#define CONFIG_FILE "/run/shm/homectrl.cnf"
#define EXCEPT_FILE "exceptions.dat"
#define CH1_ON_BIT	22
#define CH1_OFF_BIT	23
#define CH2_ON_BIT	24
#define CH2_OFF_BIT	25
...
#define GET_PRECIP WEXITSTATUS(system("/home/pi/homectrl/precip.sh"))
...
static void sprinkler(void)
{
	...
	case ST_IDLE:
		...
		if(difftime(t, sp_last_t) >= sp_freq*60*60*24 && ptm->tm_hour == SP_START_HOUR){
		    ...
		    precip = GET_PRECIP; // first check precip forecast
		    if(precip > 5){
				sp_st = ST_SKIP;
			}
		    else{
	...
	case ST_ON: // wait for sprinkler time
		if(difftime(t, saved_t) >= (SP_ON_TIME + sp_time_corr[sp_round-1])){
...
int main(void)
{
	...
	printf("HomeControl V3.0 - Starting on %d/%d/%d\n", ptm->tm_year+1900, ptm->tm_mon+1, ptm->tm_mday);

    // Set up gpi pointer for direct register access
    setup_io();
    // Set GPIO pins to output
    INP_GPIO(CH1_ON_BIT); // must use INP_GPIO before we can use OUT_GPIO
    OUT_GPIO(CH1_ON_BIT);
    INP_GPIO(CH1_OFF_BIT);
    OUT_GPIO(CH1_OFF_BIT);
    INP_GPIO(CH2_ON_BIT);
    OUT_GPIO(CH2_ON_BIT);
    INP_GPIO(CH2_OFF_BIT);
    OUT_GPIO(CH2_OFF_BIT);
	...
	for(; ; sleep(30)){
		...
		case 18:	// 18 UTC: log daily temperatures
			if(T_flag){ // do it only once
				T_flag = 0;
				if((fc = fopen("/run/shm/T.dat", "a")) != NULL){
		...
		if((fc = fopen("/run/shm/homectrl.stat", "w")) == NULL){

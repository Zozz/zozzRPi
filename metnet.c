...
int main(int argc, char *argv[])
{
    ...
    struct hostent *host;
    char keygen[] = "echo -n 'ZWMRSCE323B11B2E18FC9C1DB1DA870BACAA82010-05-29 21:14:10' | md5sum >/run/shm/key"; //sector
	...
	if((fc = fopen("statid", "r")) != NULL){
		fgets(&keygen[9], 38, fc);
		fclose(fc);
	}
	else{
		syslog(LOG_INFO, "statid: %m");
		exit(EXIT_FAILURE);
	}
	...
	for(; ; sleep(interval)){
		...
		system(keygen);
		key[0] = '\0';
		if((fc = fopen("/run/shm/key", "r")) != NULL){
			fgets(key, 33, fc);
			fclose(fc);
		}
		strupr(key);
